    ...
    w = class {
        ...
        static calculateControlPoints(t, e, i) {
            let n = t.x - e.x,
                ...
                y = e.y - v.y;
            return {
                c1: new _(h.x + x, h.y + y),
                c2: new _(a.x + x, a.y + y)
            }
        }
        length() {
            let e = 0,
            ...
            for (let s = 0; s <= 10; s += 1) {
                let o = s / 10,
                    r = this.point(o, this.startPoint.x, this.control1.x, this.control2.x, this.endPoint.x),
...
var f = class extends p {
    ...
    toDataURL(t = "image/svg+xml;base64", e) {
        switch (t) {
            case "image/svg+xml":
    ...
    _strokeUpdate(t) {
        ...
        let e = t.clientX,
            ...
            a = h ? s.distanceTo(h) <= this.minDistance : !1,
    ...
    _addPoint(t, e) {
        ...
        if (i.push(t), i.length > 2) {
            ...
            let n = this._calculateCurveWidths(i[1], i[2], e),
                s = w.fromPoints(i, n);
            return i.shift(), s
        }
        return null
        ...
        for (let n of t) {
            ...
            if (s.length > 1)
                for (let r = 0; r < s.length; r += 1) {
                    let h = s[r],
                        a = new _(h.x, h.y, h.pressure, h.time);
                    r === 0 && this._reset(o);
    ...
    toSVG({
        ...
        let e = this._data,
            ...
            n = 0,
            s = 0,
            o = this.canvas.width / i,
            r = this.canvas.height / i,
        ...
        return this._fromData(e, (a, {
            ...
            let l = document.createElement("circle"),
                v = c > 0 ? c : (u + m) / 2;
...
function E(g, t) {
    return {
        ...
        getCanvasOffsetDimensions() {
            let e = this.canvas,
                i = e.cloneNode(!0);
            if (e.offsetHeight > 0 && e.offsetWidth > 0) return {
                height: e.offsetHeight,
                width: e.offsetWidth
            };
            i.style.visibility = "hidden", document.body.appendChild(i);
            let n = i.offsetHeight,
                s = i.offsetWidth;
